...
def compile_act(policy, preprocess, precision: str, calib_cache: str | Path | None, out_dir: Path,
    ...
    if xml.exists():
        ...
        elif precision == "w4":  # weight-only INT4, groups of 64: a study. Layers whose input size is not a multiple of
            # 64 (the state, skill and latent projections: 12, 5 and 32 inputs) keep the INT8 backup precision.
            from nncf.quantization.advanced_parameters import AdvancedCompressionParameters

            model = nncf.compress_weights(
                model, mode=nncf.CompressWeightsMode.INT4_ASYM, group_size=64, ratio=1.0,
                advanced_parameters=AdvancedCompressionParameters(
                    group_size_fallback_mode=nncf.GroupSizeFallbackMode.IGNORE))
        elif precision != "fp32":

...
second Speechmatics text-to-speech voice, deliberately different from the robot's, so the two are never confused
(`scripts/voice_over.py`). The robot then answers in its own Speechmatics voice in every run — the plan it chose, the
reason for a correction, and what it could not do.*
